...
## Data Sources
We collected data from three primary sources: StatsAPI, which we accessed through a custom scraper; MLB.com, which we scraped using an automated web browser to handle dynamically loaded content; and Baseball Savant, from which we downloaded static CSV files.
...
## Data Organization
In the end the data was organized and combined using third normal form (3NF) to minimize redundancy and maintain reliable links across related data. Our entity-relationship diagram (ERD) provides a clear visual representation, clearly mapping the relationships among all entities derived from our data acquisition process.
